...
public partial class SuperCharacterController : MonoBehaviour
{
    ...
    [SerializeField] private bool fixedTimeStep = false;

    [SerializeField] private int fixedUpdatesPerSecond = 0;

    [SerializeField] private bool clampToMovingGround = true;

    [SerializeField] private bool debugSpheres = true;

    [SerializeField] private bool debugGrounding = true;

    [SerializeField] private bool debugPushbackMesssages = true;
    ...
    private const string TemporaryLayer = "Default";
    ...
    private void Awake()
    {
        ...
        {
            defaultCollisionType = new GameObject("DefaultSuperCollisionType", typeof(SuperCollisionType))
                .GetComponent<SuperCollisionType>();
        }
    ...
    private void LateUpdate()
    {
        ...
        if (clampToMovingGround && isClamping && clampedTo != null &&
            clampedTo.position - lastGroundPosition != Vector3.zero)
        {
    ...
    private void RecursivePushback(int depth, int maxDepth)
    {
        ...
        foreach (CollisionSphere sphere in spheres)
        {
                         ...
                         triggerInteraction))
            {
                ...
                bool contactPointSuccess =
                    SuperCollider.ClosestPointOnSurface(col, position, radius, out Vector3 contactPoint);
