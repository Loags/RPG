...
namespace LB
{
    public class RPGCharacterMovementController : SuperStateMachine
    {
        ...
        [Header("Debug Options")] public bool debugMessages;
        ...
        private void Awake()
        {
            ...
            rpgCharacterController.SetHandler(HandlerTypes.AcquiringGround,
                new SimpleActionHandler(() => { }, () => { }));
            rpgCharacterController.SetHandler(HandlerTypes.MaintainingGround,
                new SimpleActionHandler(() => { }, () => { }));
            ...
            rpgCharacterController.SetHandler(HandlerTypes.Sprint, new Sprint(this));
        }

        private void Start()
        {
            m_YAxisMaxSpeed = cinemachineFreeLook.m_YAxis.m_MaxSpeed;
            m_XAxisMaxSpeed = cinemachineFreeLook.m_XAxis.m_MaxSpeed;
            ...
            rpgCharacterController.OnLockCursor += LockCursor;
            rpgCharacterController.OnUnLockCursor += UnLockCursor;
            rpgCharacterController.OnLockCamera += LockCamera;
            rpgCharacterController.OnUnLockCamera += UnLockCamera;

            LockCursor();
            UnLockCamera();

            RPGCharacterAnimatorEvents animatorEvents =
                rpgCharacterController.GetAnimatorTarget().GetComponent<RPGCharacterAnimatorEvents>();
        ...
        */
        ...
        protected override void LateGlobalSuperUpdate()
        {
            ...
            if (rpgCharacterController.CanMove)
            {
                if (currentVelocity.magnitude > 0f)
                {
                    ...
                    animator.SetFloat(AnimationParameters.VelocityZ,
                        transform.InverseTransformDirection(currentVelocity).z * movementAnimationMultiplier);
            ...
            animator.SetFloat(AnimationParameters.VelocityX,
                transform.InverseTransformDirection(currentVelocity).x * movementAnimationMultiplier);
            animator.SetFloat(AnimationParameters.VelocityZ,
                transform.InverseTransformDirection(currentVelocity).z * movementAnimationMultiplier);
        }
        ...
        private void Idle_SuperUpdate()
        {
            ...
            if (rpgCharacterController.rpgCharacterInputSystemController.HasSprintInput())
                rpgCharacterController.TryStartAction(HandlerTypes.Sprint);
        }
        ...
        // Run every frame character is moving.
        private void Sprint_SuperUpdate()
        {
            if (debugMessages)
            {
                // Debug.Log("Sprint_SuperUpdate");
            }
            ...
            // Set speed determined by movement type.
            if (rpgCharacterController.CanMove && rpgCharacterController.CanSprint)
            {
                float moveSpeed = runSpeed * 2f;
                float moveAccel = runAccel * 2f;

                if (rpgCharacterController.IsStrafing)
                {
                    moveSpeed = walkSpeed * 1.5f;
                    moveAccel = walkAccel * 1.5f;
                }
            ...
            // Return to move state when no longer sprinting
            rpgCharacterController.TryStartAction(HandlerTypes.Move);
        }
        ...
        private void Jump_SuperUpdate()
        {
            ...
            {
                Vector3 destination = new Vector3(currentVelocity.x, (jumpSpeed / 4f), currentVelocity.z);
                currentVelocity = Vector3.MoveTowards(currentVelocity, destination,
        ...
        private void DiveRoll_SuperUpdate()
        {
            if (debugMessages)
            {
                // Debug.Log("DiveRoll_SuperUpdate");
            }
        ...
        /// <summary>
        /// Lock the cursor and set is invisible
        /// </summary>
        ...
        /// <summary>
        /// Unlock the camera input
        /// </summary>
        private void UnLockCamera()
        {
            cinemachineFreeLook.m_XAxis.m_MaxSpeed = m_XAxisMaxSpeed;
            cinemachineFreeLook.m_YAxis.m_MaxSpeed = m_YAxisMaxSpeed;
        }
    }
}