...
public class EnemyHitBox : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (hitObjects.Count <= 0 || hitObjects.Contains(other.gameObject))
            return; // Skip when the GameObject is already in the list
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (hitObjects.Count <= 0 || !hitObjects.Contains(other.gameObject))
            return; // Skip when the GameObject is not in the list
    ...
    {
        return hitObjects;
    } // Make the List accessable to other scripts
