...
public class EnemyCombat : MonoBehaviour
{
    ...
    [Space(15)] [Header("Attack stats")] [SerializeField]
    ...
    [Space(15)] [Header("Health stats")] [SerializeField]
    private float maxHealth;

    [SerializeField] private float currentHealth;
    [SerializeField] private float healthRegenRate;

    [Space(15)] [Header("Stamina stats")] [SerializeField]
    ...
    [Space(15)] [Header("Defensive stats")] [SerializeField]
    ...
    private void CheckAttackState()
    {
        ...
        if (inAttackRangeCoroutine == null)
            SM_EnemyController.InitializeCoroutine(ref inAttackRangeCoroutine,
                InAttackRangeCheck()); // Start inAttackRangeCheck
    ...
    private void EndInAttackRangeCheck()
    {
        ...
        SM_EnemyController.TerminateCoroutine(ref inAttackRangeCoroutine,
            InAttackRangeCheck()); // End inAttackRangeCheck
    }
    ...
    public IEnumerator Attack()
    {
        ...
        SM_EnemyController.InitializeCoroutine(ref attackCooldownCoroutine,
            AttackCooldown()); // Start the cooldown for AIs attack
    ...
    private IEnumerator InAttackRangeCheck()
    {
        ...
        while (continueLoop)
        {
            ...
            if (Vector3.Distance(this.transform.position,
                    SM_EnemyController.SM_EnemyTargeting.currentTarget.transform.position) <
                attackRange) // Check the distance from ai to current target
            {
    ...
    private void ApplyDamage(float _damage)
    {
        ...
        {
            if (_target.TryGetComponent(out RPGCharacterController otherController))
            {
